...
impl<F: IsFFTField> CommonPreprocessedInput<F> {
    pub fn from_constraint_system(
        ...
        Self {
            ...
            ql: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&ql).unwrap(), // TODO: Remove unwraps
            qr: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&qr).unwrap(),
            qo: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&qo).unwrap(),
            qm: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&qm).unwrap(),
            qc: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&qc).unwrap(),
            s1: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&s1_lagrange).unwrap(),
            s2: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&s2_lagrange).unwrap(),
            s3: <Polynomial<FieldElement<F>> as FFTPoly<F, F>>::interpolate_fft(&s3_lagrange).unwrap(),
            s1_lagrange,
            s2_lagrange,
            s3_lagrange,
        }
